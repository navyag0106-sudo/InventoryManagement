...
(function(window) {
    ...
    window.QRCode = function(targetElement, options) {
        ...
        var opts = {
            ...
            correctLevel: options.correctLevel || 1, // L:1, M:0, Q:3, H:2
            text: options.text || ""
        };

        var model = new QRCodeModel(0, opts.correctLevel);
        model.addData(opts.text);
        model.make();
        ...
        for (var row = 0; row < count; row++) {
            for (var col = 0; col < count; col++) {
                ctx.fillStyle = model.isDark(row, col) ? opts.colorDark : opts.colorLight;
